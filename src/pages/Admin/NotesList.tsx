...
export default function NotesList() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
          <button
            ...
            className={cn(
              ...
              viewMode === 'grid' ? "bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-indigo-400" : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"
            )}
          ...
          <button
            ...
            className={cn(
              "p-2 rounded-lg transition-all",
              viewMode === 'list' ? "bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-indigo-400" : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"
            )}
            ...
            <div className={cn(
              ...
              {filteredNotes.map((note) => (
                <motion.div
                  ...
                  className={cn(
                    "group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 transition-all hover:shadow-md relative cursor-pointer",
                    viewMode === 'grid' ? "rounded-2xl p-6" : "rounded-xl p-4 flex flex-col gap-3"
                  )}
                >
                  <div className="flex flex-col gap-1 flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className={cn(
                        ...
                        viewMode === 'grid' ? "text-lg mb-2" : "text-sm"
                      )}>
                        {note.title}
                      </h3>
                      <div className={cn(
                        "flex items-center gap-1 transition-opacity shrink-0",
                        viewMode === 'grid' ? "opacity-0 group-hover:opacity-100" : "opacity-0 group-hover:opacity-100"
                      )}>
                        <button
                          onClick={(e) => { e.stopPropagation(); handleOpenModal(note); }}
                          className="p-1.5 text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/10 rounded-lg transition-colors"
                        >
                    ...
                    <p className={cn(
                      ...
                      viewMode === 'grid' ? "text-sm mb-6 line-clamp-6" : "text-xs line-clamp-2"
                    )}>
                      {note.content}
                    </p>
                  </div>
                  ...
                  <div className={cn(
                    viewMode === 'grid' 
                      ? "pt-4 border-t border-slate-100 dark:border-slate-800 space-y-2" 
                      : "flex items-center justify-between pt-2 border-t border-slate-50 dark:border-slate-800/50"
                  )}>
                    <div className="flex items-center gap-2 text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-tight">
                      ...
                      <span className="truncate max-w-[100px]">{getClienteName(note.cliente_id)}</span>
                    </div>
                    <div className="flex items-center gap-2 text-[10px] font-medium text-slate-500 dark:text-slate-400">
                      <CalendarIcon className="w-3 h-3" />
                      {(() => {
                        try {
                          return note.date ? format(new Date(note.date + 'T12:00:00'), "dd/MM/yy") : "--/--/--";
                        } catch (e) {
                          return "!!/!!/!!";
                        }
                      })()}
      ...
      {viewingNote && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md" onClick={() => setViewingNote(null)}>
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200 overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="p-8 space-y-6">
              <div className="flex items-start justify-between gap-4">
                ...
                <button onClick={() => setViewingNote(null)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-2xl transition-all"><X className="w-8 h-8 text-slate-400" /></button>
              </div>
              ...
              <div className="flex items-center gap-6 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-slate-700 dark:text-slate-200">{getClienteName(viewingNote.cliente_id)}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-slate-700 dark:text-slate-200">
                      {format(new Date(viewingNote.date + 'T12:00:00'), "dd 'de' MMMM, yyyy", { locale: ptBR })}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-slate-100 dark:border-slate-800 flex gap-4">
                <button
                  onClick={() => { handleOpenModal(viewingNote); setViewingNote(null); }}
                  className="flex-1 flex items-center justify-center gap-2 py-4 bg-slate-100 dark:bg-slate-800 hover:bg-indigo-500 hover:text-white text-slate-700 dark:text-slate-200 font-black uppercase text-xs tracking-widest rounded-2xl transition-all shadow-sm"
                >
                ...
                <button
                  ...
                  className="flex-1 py-4 bg-indigo-600 text-white font-black uppercase text-xs tracking-widest rounded-2xl transition-all shadow-xl shadow-indigo-500/20"
                >
                  Fechar
                </button>
              </div>
